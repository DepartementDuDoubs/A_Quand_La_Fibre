...
function onLocationFound(e) {
    ...
    .bindPopup("Vous êtes géolocalisé ici "+ adresse.value + " avec une marge d'erreur de " + radius+ " mètres.<br>Cliquez sur Envoyer pour valider, sinon modifiez l'adresse dans le champ prévu.<br>lat : "+ lati + "<br>lon : "+longi ).openPopup();
...
function getCoord(){
    $.ajax({
        ...
        success: function(reponse) {
            ...
            .bindPopup("Vous êtes ici " + adressePostale + '<br>lat : ' + lati + '<br>lng : ' + longi ).openPopup();
            // On recentre la map
            mymap.setView(
                reponse,
                15
...
function getDate1() {
    $.ajax({
        ...
        success: function(data1) {
            ...
            if (data1 == undefined) {
                $('#final').html("Adresse introuvable dans notre base.<br>Vous ne dépendez pas de la zone du Réseau d'Initiative Publique.");
            } else {
    ...
    $.ajax({
        ...
        success: function(data2) {
            ...
            if ((date1 == '2021-2022') && (date2 >> 2022)) {
                ...
                $('#final').html("Vous devriez avoir la fibre chez vous en "+ date1+ ".<br> Le Département du Doubs vous a fait gagner "+ tempsGagne+ " an(s) sur la date initialement prévue.");
            } else {
                $('#final').html('Vous devriez avoir la fibre en '+date1 + '.');
            }
        },
        error: function() {
